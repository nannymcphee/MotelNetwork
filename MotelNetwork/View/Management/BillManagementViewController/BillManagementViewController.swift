...
class BillManagementViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func fetchUser() {
        ...
        reference.observe(.value) { (snapshot) in
            ...
            if self.userType == 0 {
                self.loadDataForOwner3().continueWith(continuation: { (task) -> Void in
                    if task.error != nil {
                        print(task.result?.toJSONString(prettyPrint: true))
                    } else {
    ...
    func loadDataForOwner3() -> Task<[Bill]> {
        let task = TaskCompletionSource<[Bill]>()
        let ref = Database.database().reference()
        let myRecentBillsQuery = ref.child("Bills").queryOrdered(byChild: "ownerID").queryEqual(toValue: uid)
        ...
        myRecentBillsQuery.observe(.childAdded, with: { (snapshot) in
            if let json = snapshot.value as? [String: AnyObject] {
                ...
                bill = Mapper<Bill>().map(JSON: json) ?? Bill()
        ...
        task.set(result: self.listBill)

        
        return task.task
    }
    
    func loadDataForOwner2() {
        let ref = Database.database().reference()
        let myRecentBillsQuery = ref.child("Bills").queryOrdered(byChild: "ownerID").queryEqual(toValue: uid)
        
        myRecentBillsQuery.keepSynced(true)
