...
class EditRoomViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate {
    ...
    func setUpView() {
        ...
        ivRoomImage0.layer.cornerRadius = 10
        ivRoomImage1.layer.cornerRadius = 10
        ivRoomImage2.layer.cornerRadius = 10
        
        if currentRoom.roomImageUrl0 != nil || currentRoom.roomImageUrl1 != nil || currentRoom.roomImageUrl2 != nil {
            let resource0 = ImageResource(downloadURL: URL(string: currentRoom.roomImageUrl0!)!)
            let resource1 = ImageResource(downloadURL: URL(string: currentRoom.roomImageUrl1!)!)
            let resource2 = ImageResource(downloadURL: URL(string: currentRoom.roomImageUrl2!)!)
            
            ivRoomImage0.kf.setImage(with: resource0, placeholder: #imageLiteral(resourceName: "defaultImage"), options: nil, progressBlock: nil, completionHandler: nil)
            ivRoomImage1.kf.setImage(with: resource1, placeholder: #imageLiteral(resourceName: "defaultImage"), options: nil, progressBlock: nil, completionHandler: nil)
            ivRoomImage2.kf.setImage(with: resource2, placeholder: #imageLiteral(resourceName: "defaultImage"), options: nil, progressBlock: nil, completionHandler: nil)
        } else {
            ivRoomImage0.image = #imageLiteral(resourceName: "defaultImage")
            ivRoomImage1.image = #imageLiteral(resourceName: "defaultImage")
            ivRoomImage2.image = #imageLiteral(resourceName: "defaultImage")
        }
    ...
    @IBAction func btnSavePressed(_ sender: Any) {
        ...
        if area.isEmpty || address.isEmpty || price.isEmpty || roomName.isEmpty || usersAllowed.isEmpty {
            ...
            let values = ["roomName": roomName, "area": area, "price": priceDouble ?? 0.0, "ownerID": ownerID ?? "", "renterID": renterID ?? "", "roomImageUrl0": roomImageUrl0, "roomImageUrl1": roomImageUrl1, "roomImageUrl2": roomImageUrl2, "usersAllowed": usersAllowed, "address": address, "renterName": renterName] as [String : AnyObject]
                    ...
                    if renterName.isEmpty {
                        ...
                        for image in self.imageArray {
                            self.uploadImage(image: image) { (url) -> (Void) in
                                ...
                                for url in self.urlArray {
                                    let values = ["roomName": roomName, "area": area, "price": priceDouble ?? 0.0, "ownerID": ownerID ?? "", "roomImageUrl\(self.urlArray.index(of: url) ?? 0)": url, "usersAllowed": usersAllowed, "address": address, "renterName": "", "renterID": ""] as [String: AnyObject]
                        ...
                        for image in self.imageArray {
                            self.uploadImage(image: image) { (url) -> (Void) in
                                ...
                                for url in self.urlArray {
                                    let values = ["roomName": roomName, "area": area, "price": priceDouble ?? 0.0, "ownerID": ownerID ?? "", "roomImageUrl\(self.urlArray.index(of: url) ?? 0)": url, "usersAllowed": usersAllowed, "address": address, "renterName": renterName] as [String: AnyObject]
