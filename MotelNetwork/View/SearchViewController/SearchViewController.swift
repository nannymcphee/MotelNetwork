    ...
    func loadData2() {
        ...
        ref.observe(.childAdded, with: { (snapshot) in
            ...
            if let json = snapshot.value as? [String: AnyObject] {
                ...
                self.listNews.append(news)
                self.listNewsFiltered2 = self.listNews.filter({ (news) -> Bool in
                    return Int(news.area ?? "") ?? 0 <= 20
                })
    ...
    func dropdownMenu(_ dropdownMenu: DropdownMenu, didSelectRowAt indexPath: IndexPath) {
        ...
        case 1:
            if indexPath.row == 0 {
                ...
                self.listNewsFiltered2 = self.listNews.filter({ (news0) -> Bool in
                    return Int(news0.price!) ?? 0 < 3000000
                })
                ...
                self.listNewsFiltered2 = self.listNews.filter({ (news0) -> Bool in
                    return Int(news0.price!) ?? 0 >= 3000000 && Int(news0.price!) ?? 0 <= 5000000
                })
                ...
                self.listNewsFiltered2 = self.listNews.filter({ (news0) -> Bool in
                    return Int(news0.price!) ?? 0 > 5000000
                })
